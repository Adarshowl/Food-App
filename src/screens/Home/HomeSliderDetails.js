...
const HomeSliderDetails = ({ navigation, route }) => {
    ...
    const [similerP, setSimilierPro] = useState([]);
    ...
    useEffect(() => {
        ...
        console.log("<<<<<<jjjj", item?.product_link)
        ...
        (async () => {
            if (item?.product_id?._id != undefined) {
                ...
                let is_product_save = await isProductSavedFavorite(
                    item?.product_link || item?.product_id?._id,
                );
                setFav(is_product_save);
            }
        })();
    ...
    const successSimilerProdcut = async data => {
        dispatch(showProgressBar(false));
        // console.log("lll",data?.data)
        setSimilierPro(data?.response);
    };
    ...
    const [similerProdcut, setSimilerProduct] = useState([]);
    const [loading, setLoading] = useState('')
    const [showEmpty, setShowEmpty] = useState(false)

    // const idd = items?._id

    // console.log('check idd', similerP)


    // const getSimilerProduct = () => {
    //   setLoading(true);
    //   try {
    //     console.log("response axios >>> ", JSON.stringify(API_END_POINTS.API_SIMILER_PRODCUT+ idd));

    //     ApiCall('get', null, API_END_POINTS.API_SIMILER_PRODCUT + idd, {
    //       // 'Content-Type': 'application/json',
    //       // 'x-access-token': userToken 
    //     }).then(response => {
    //       console.log("response similer >>> ", JSON.stringify(response));
    //       if (response?.statusCode === 200) {
    //         // console.log("Response data: ", JSON.stringify(response.data));
    //         setSimilerProduct(response?.data?.response)
                ...
                {similerP?.length == 0 ? null : (
                    ...
                    <View>
                        ...
                        <FlatList
                            ...
                            //     : [items?.thumbnail_image]
                            // }
                            renderItem={({ item, index }) => <SimilarProductItem item={item} />}
                        />
                    </View>
                )}
                {/* <View style={styles.divideLine} /> */}
